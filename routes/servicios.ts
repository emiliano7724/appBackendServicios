...
servicioRoutes.post("/getHorariosServiciosCliente", verificarToken, async (req: any, res: Response) => {
    try {
        ...
        const result: any =
            await query("select * from horario_cliente where id_cliente=" + id);
    ...
    try {
        ...
        const result: any =
            await query("INSERT INTO `frecuencia_horaria` (" +
                ", `hiLunes`, `hfLunes`,`hiMartes`, `hfMartes`,`hiMiercoles`, `hfMiercoles`, `hiJueves`, `hfJueves`, `hiViernes`, `hfViernes`, `hiSabado`, `hfSabado`, `hiDomingo`, `hfDomingo`,`id_cliente`,`id_user` )"
                + " VALUES (?, ?, ?, ?, ?,?, ?, ?,?,?,?,?, ?,?, ?, ?, ?, ?, ?,?, ?, ?,?,?,?,?, ?,?,?, ?, ?, ?, ?,?, ?, ?,?,?,?,?, ?,?,?, ?, ?, ?, ?,?, ?, ?,?,?,?,?, ?,?,?,? )", [
